...
def prompt_to_image(workflow, positve_prompt, negative_prompt='', save_previews=False):
    ...
    # {'3': 'KSampler', '4': 'CheckpointLoaderSimple', '5': 'EmptyLatentImage', '6': 'CLIPTextEncode', '7': 'CLIPTextEncode', '8': 'VAEDecode', '9': 'SaveImage'}
    # 找到KSampler节点的ID
    k_sampler_id = next(key for key, value in id_to_class_type.items() if value == 'KSampler')
    
    # 为KSampler节点生成随机种子
    prompt[k_sampler_id]['inputs']['seed'] = random.randint(10**14, 10**15 - 1)
    
    # 获取正面提示词节点的ID
    positive_input_id = prompt[k_sampler_id]['inputs']['positive'][0]
    
    # 更新正面提示词
    prompt[positive_input_id]['inputs']['text'] = positve_prompt

    if negative_prompt != '':
        negative_input_id = prompt.get(k_sampler_id)['inputs']['negative'][0]
...
prompt_to_image(load_workflow('./workflow_base.json'), 'beautiful scenery nature glass bottle landscape, , purple galaxy bottle,', 'text, watermark', True)
